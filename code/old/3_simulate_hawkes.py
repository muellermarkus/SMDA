...
def simulate_hawkes(theta, N, T_max):
    ...
    pars = theta.copy()
    if len(pars) == 4:
        pars[1] = pars[1] * pars[3] * pars[2] ** pars[3]
    else:
        pars[1] = pars[1] * pars[2]
    ...
    while (n_events < N) and (T < T_max):
        # set upper bound
        if len(pars) == 4:
            Lambda_upper_bound = homogenous_hawkes_process(pars[0], power_law_kernel, T, vT, right = True, alpha = pars[1], delta = pars[2], eta = pars[3])
        else:
            Lambda_upper_bound = homogenous_hawkes_process(pars[0], exp_decay, T, vT, right = True, alpha = pars[1], delta = pars[2])
        ...
        if len(pars) == 4:
            LambdaT = homogenous_hawkes_process(pars[0], power_law_kernel, T, vT, alpha = pars[1], delta = pars[2], eta = pars[3])
        else:
            LambdaT = homogenous_hawkes_process(pars[0], exp_decay, T, vT, alpha = pars[1], delta = pars[2])
...
def plot_hawkes(theta, vT):
    ...
    for T in vT:
        ...
        if len(pars) == 4:  
            l = homogenous_hawkes_process(pars[0], power_law_kernel, T, prev_times, right = True, alpha = pars[1], delta = pars[2], eta = pars[3])
        else:
            l = homogenous_hawkes_process(pars[0], exp_decay, T, prev_times, right = True, alpha = pars[1], delta = pars[2])
...
def nll_exp(theta, vT):
    ...
    for i,T in enumerate(vT):
        ...
        prev_T = [time for time in vT if time < T]
        sum_list.append(np.log(homogenous_hawkes_process(theta[0], exp_decay, T, prev_T, right = False, alpha = alpha, delta = theta[2])))
    ...
    for T in vT:
        sum_part = (alpha / theta[2]) * (1 - np.exp(-theta[2] * (last_T - T)))
        sum_list.append(sum_part) 
    
    integral_part = np.sum(sum_list)
    
    nll = -(sum_log_lambda - theta[0]*last_T - integral_part)
...
def transform(theta):
    ...
    if len(theta) == 4:
        eta = theta[3]
        eta_tr = np.log(eta - 0)
        return [l0_tr, alphaf_tr, delta_tr, eta_tr]

    return [l0_tr, alphaf_tr, delta_tr]
...
def TransformBack(P_tr):
    ...
    # delta = (5 - 0)*aux + 0
    delta = np.exp(P_tr[2]) + 0
    
    if len(P_tr) == 4:
        eta = np.exp(P_tr[3]) + 0
        return [l0, alphaf, delta, eta]
    return [l0, alphaf, delta]
...
lik_func = lambda P_tr: nll(TransformBack(P_tr), vT_dis)/len(vT_dis)
par0 = transform([1.0,0.4,0.8,0.8])
